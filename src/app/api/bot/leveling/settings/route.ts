...
export async function GET(req: Request) {
  ...
  try {
    const data = await botApi(`/api/leveling/settings/${guildId}`, { userId: (session.user as any).id });
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!guildId) return NextResponse.json({ error: "Missing guildId" }, { status: 400 });
    const result = await botApi(`/api/leveling/settings/${guildId}`, { method: "POST", body: { settings }, userId: (session.user as any).id });
